...
{
    (query1, k_search, l_search, indices, distances, beam_width, std::numeric_limits<uint32_t>::max(),
        ...
        if (!frontier.empty())
        {
            ...
            for (uint64_t i = 0; i < frontier.size(); i++)
            {
                ...
                frontier_read_reqs.emplace_back(get_node_sector((size_t)id) * defaults::SECTOR_LEN,
                                                num_sectors_per_node * defaults::SECTOR_LEN, fnhood.second);   // key
                if (stats != nullptr)
                {
                    stats->n_4k++;
                    stats->n_ios++;
...
#else
            reader->read(frontier_read_reqs, ctx); // synchronous IO linux      // key func
#endif
            if (stats != nullptr)
            {
